...
func (pm *PluginManager) LoadPlugin(pluginPath string) error {
	...
	pluginID, err := GetPluginIdDB(manifest.Name, manifest.Version, manifest.Description)
	if err != nil {
...
func (pm *PluginManager) registerPluginRoutes(plugin *Plugin) {
	...
	for _, route := range plugin.Manifest.Routes {
		...
		pm.routeMutex.Lock()
		pm.registeredRoutes[plugin.ID] = append(pm.registeredRoutes[plugin.ID],
...
func (pm *PluginManager) RegisterPlugin(plugin *Plugin) {
	...
	exist, err := CheckPluginWithInfo(plugin.Manifest.Name, plugin.Manifest.Version, plugin.Manifest.Description)
	if err != nil {
	...
	var pluginID int
	if !exist {
		pluginID, err = AddPluginToDB(plugin.Manifest.Name, plugin.Manifest.Version, true, plugin.Manifest.Description, 0, "active")
		if err != nil {
			log.LogError("Failed to add plugin to database", zap.Error(err))
		}
	} else {
		pluginID, err = GetPluginIdDB(plugin.Manifest.Name, plugin.Manifest.Version, plugin.Manifest.Description)
	...
	if _, exists := pm.plugins[plugin.ID]; exists {
		log.LogWarn("Plugin already registered", zap.String("plugin", plugin.Manifest.Name))
		return
	}
	...
	log.LogInfo("Plugin registered successfully", zap.String("plugin", plugin.Manifest.Name))
}
...
func (pm *PluginManager) EnablePlugin(pluginID int) error {
	...
	log.LogInfo("Plugin enabled successfully", zap.String("plugin", plugin.Manifest.Name))
	return nil
}
...
func (pm *PluginManager) DisablePlugin(pluginID int) error {
	...
	log.LogInfo("Plugin disabled successfully", zap.String("plugin", plugin.Manifest.Name))
	return nil
}
...
func (pm *PluginManager) UninstallAllPlugins() error {
	...
	for id, plugin := range pm.plugins {
		...
		log.LogInfo("Plugin uninstalled successfully", zap.String("plugin", plugin.Manifest.Name))
	}
