...
func main() {
	...
	pluginService := services.NewPluginService(router)

	// Initialize plugin handlers with logger BEFORE setting up routes
	api.InitializePluginHandlers(pluginService.Manager, logger)

	pluginService.AutoLoadPlugins()

	// Setup all routes (including plugin routes)
	routes.SetupRoutes(router, pluginService)
