...
const arcLabel =
  (_data: { value: number }[], total: number) =>
  ({ value }: { value: number }) => {
  ...
  return (
    <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
      ...
      {loading ? (
        ...
        <div className="flex h-40 items-center justify-center text-gray-400">No data</div>
      ) : (
                ...
                <ul className="mt-4 flex flex-wrap justify-center gap-4">
                  {payload &&
                    payload.map((entry, idx) => (
                      <li
                        key={`legend-item-${idx}`}
                        className="flex items-center gap-3 rounded px-3 py-1 text-base text-gray-700 dark:text-gray-200"
                        style={{ cursor: 'default' }}
                      >
                        <span
                          className="inline-block h-4 w-4 rounded-full border border-gray-300 dark:border-gray-700"
...
const MetricsPieCharts = () => {
  ...
  const { data: httpErrorRequestsTotal, isLoading: httpErrorRequestsLoading } =
    useHttpErrorRequestsTotal();
