...
export const GitHubTab = ({ onComplete }: Props) => {
  ...
  const renderForm = () => {
    switch (selectedOption) {
      case "popular":
        return <PopularRepositoriesForm onSelect={handleRepoSelect} theme={isDarkTheme ? "dark" : "light"} />;
      case "yourGitHub":
        return (
    ...
    <Box sx={{ display: "flex", flexDirection: "column", width: "100%", gap: 3 }}>
      ...
      <Box sx={{ display: "flex", gap: 3, flexDirection: { xs: "column", md: "row" } }}>
        <Card sx={{ 
          ...
          borderRadius: "12px",
          boxShadow: isDarkTheme ? "0 4px 14px rgba(0, 0, 0, 0.3)" : "0 4px 14px rgba(0, 0, 0, 0.1)"
        }}>
