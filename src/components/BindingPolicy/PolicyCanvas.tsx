...
const PolicyCanvas: React.FC<PolicyCanvasProps> = ({
  ...
  const throttledUpdateConnectionLines = useCallback(
    (newLines: typeof connectionLines) => {
    ...
    const renderConnections = () => {
      ...
      policyCanvasEntities.workloads.forEach(workloadName => {
        policyCanvasEntities.clusters.forEach(clusterName => {
          ...
          const connection = connectionLines.find(
            conn =>
              (conn.source === `workload-${workloadName}` &&
                conn.target === `cluster-${clusterName}`) ||
              (conn.source === `cluster-${clusterName}` &&
                conn.target === `workload-${workloadName}`)
          );
      ...
      if (activeConnection.source && activeConnection.sourceType) {
        ...
        if (sourceElement) {
          ...
          const startX =
            activeConnection.sourceType === 'workload'
  ...
  const handleCanvasItemClick = useCallback(
    (itemType: 'policy' | 'cluster' | 'workload', itemId: string) => {
      ...
      console.log(
        `🔄 Canvas item clicked: ${itemType}-${itemId}, activeConnection:`,
        activeConnection
      );
      ...
      if (!activeConnection.source) {
        ...
        if (
          ...
          console.log(
            `✅ Connection completed between workload ${workloadId} and cluster ${clusterId}`
          );
          ...
          setInvalidConnectionWarning(
            `Cannot connect ${sourceType} to ${itemType}. Please connect a workload to a cluster.`
          );
  ...
  const getWorkloadsForLabel = (workloads: Workload[], key: string, value: string): Workload[] => {
    return workloads.filter(workload => workload.labels && workload.labels[key] === value);
  };

  // Find all clusters that match a given label
  const getClustersForLabel = (
    clusters: ManagedCluster[],
    key: string,
    value: string
  ): ManagedCluster[] => {
    return clusters.filter(cluster => cluster.labels && cluster.labels[key] === value);
  };
  ...
  const handleCanvasGlobalClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      <StrictModeDroppable droppableId="canvas" type="CLUSTER_OR_WORKLOAD">
        {(provided, snapshot) => (
          ...
          >
            ...
            {/* Empty state */}
            {isCanvasEmpty && !snapshot.isDraggingOver && renderEmptyState()}
                  ...
                  >
                    ...
                    >
                      {policyCanvasEntities.clusters.length > 0 ? (
                        ...
                        >
                          ...
                          {policyCanvasEntities.clusters.map(clusterId => {
                            ...
                            if (matchingClusters.length === 0) {
                              console.log(
                                `No matching clusters found for ${clusterId}, using label info directly`
                              );
                              if (labelInfo) {
                                return (
                                  <Paper
                        ...
                        >
                          Select clusters to add them to the canvas
                        </Typography>
                      )}
                    </Box>
                  </Box>
                  ...
                  >
                    ...
                    >
                      {policyCanvasEntities.workloads.length > 0 ? (
                        ...
                        >
                          ...
                          {policyCanvasEntities.workloads.map(workloadId => {
                            ...
                            if (matchingWorkloads.length === 0) {
                              console.log(
                                `No matching workloads found for ${workloadId}, using label info directly`
                              );

                              if (labelInfo) {
                                if (isNamespaceLabel(labelInfo)) {
                                  console.log(
                                    `Rendering namespace label: ${labelInfo.key}=${labelInfo.value}`
                                  );
                        ...
                        >
                          Select workloads to add them to the canvas
                        </Typography>
                      )}
                    </Box>
                  </Box>
          ...
          sx={{
            ...
            '&:hover': {
              ...
              backgroundColor: isDarkTheme
                ...
                : alpha(muiTheme.palette.error.main, 0.05),
