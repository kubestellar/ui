...
import { Box, Typography, Paper, IconButton, Chip, Divider, alpha, Badge } from '@mui/material';
...
const CanvasItems: React.FC<CanvasItemsProps> = ({
  ...
  const getConnectionBorderStyle = (
    ...
    return {
      ...
      cursor:
        connectMode && (itemType === 'cluster' || itemType === 'workload') ? 'pointer' : 'default',
    };
  };
  ...
  const captureItemClick = useCallback(
    (e: React.MouseEvent, itemType: 'policy' | 'cluster' | 'workload', itemId: string) => {
      e.stopPropagation();
      e.preventDefault();
            ...
            {canvasEntities.clusters.map(clusterId => {
              ...
              return (
                <Paper
                  ...
                  sx={{
                    p: 1,
                    ...getConnectionBorderStyle(
                      'cluster',
                      clusterId,
                      isDarkTheme ? '#90caf9' : '#2196f3'
                    ),
                    display: 'flex',
                    flexDirection: 'column',
                    width: 'fit-content',
                      ...
                      {labelInfo && matchingClusters.length > 1 && (
                        ...
                        >
                          <KubernetesIcon
                            type="cluster"
                            size={16}
                            sx={{ color: isDarkTheme ? '#90caf9' : undefined }}
                          />
                        </Badge>
                      )}
